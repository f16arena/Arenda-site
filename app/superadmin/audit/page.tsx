  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Журнал платформы</h1>
          <p className="text-sm text-slate-500 mt-0.5">Последние {logs.length} действий по всем организациям</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500">Организация</th>
              ...
              return (
                <tr key={l.id} className="border-b border-slate-50">
                  ...
                  <td className="px-4 py-2.5">
                    {org ? (
                      <span className="text-xs text-slate-700 font-mono">{org.slug}</span>
                    ) : (
                  ...
                  <td className="px-4 py-2.5">
                    {l.userName ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-sm font-medium text-slate-900">{l.userName}</span>
                        {l.userRole && <span className="text-[10px] text-slate-400">({l.userRole})</span>}
                      </div>
                    ) : <span className="text-slate-400">Система</span>}
                  </td>
                  <td className="px-4 py-2.5">
                    <span className={cn("inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium", ACTION_COLORS[l.action] ?? "bg-slate-100 text-slate-600")}>
                      {l.action}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-slate-600 text-xs">{l.entity}</td>
