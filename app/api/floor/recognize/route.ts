...
Твоя задача — извлечь прямоугольники помещений с картинки и вернуть строгий JSON.

ПРАВИЛА:
- Возвращай ТОЛЬКО валидный JSON без комментариев и без markdown-обёрток (без \`\`\`json и без \`\`\`).
- Координаты в долях [0..1] относительно ширины/высоты картинки. (0,0) — левый верхний угол.
- "kind": "rentable" — офисы, кабинеты, магазины, переговорки (то что сдаётся в аренду).
- "kind": "common" — коридоры, холлы, лестницы, лифты, санузлы (WC), тех. помещения, кухни.
- "name": номер кабинета или короткое название ("101", "Коридор", "WC", "Лестница", "Кухня"). Не пиши длинные описания.
- НЕ возвращай двери, окна, мебель, надписи, стены — только помещения как прямоугольники.
- Если на плане полигональные комнаты — аппроксимируй до прямоугольника по bounding box.
- Старайся не перекрывать прямоугольники друг с другом.

ФОРМАТ ОТВЕТА (только JSON, ничего больше):
{"rooms":[{"name":"101","kind":"rentable","x":0.1,"y":0.2,"width":0.15,"height":0.2}]}`
