...
export async function POST(req: Request) {
  ...
  // Ошибки читает админ в приложении — язык из его профиля (cookie нет).
  ...
  if (!tenantId) return mobileError(t("adminDocs.api.requests.tenantRequired"))
  if (title.length < 3) return mobileError(t("adminDocs.api.requests.titleRequired"))
  if (description.length < 5) return mobileError(t("adminDocs.api.requests.descriptionRequired"))
  if (!REQUEST_TYPE_SET.has(type)) return mobileError(t("adminDocs.api.requests.badType"))
  if (!REQUEST_PRIORITY_SET.has(priority)) return mobileError(t("adminDocs.api.requests.badPriority"))

  const tenant = await db.tenant.findFirst({
    where: { id: tenantId, ...tenantInBuildingsWhere(result.buildingIds) },
    select: { id: true, companyName: true, userId: true },
  })
  if (!tenant) return mobileError(t("adminDocs.api.common.tenantUnavailable"), 403)
  ...
  if (tenant.userId) {
    // Уведомление читает арендатор — заголовок на его языке.
    const { t: tTenant } = await getTForUser(tenant.userId)
    await notifyUser({
      userId: tenant.userId,
      type: "REQUEST_CREATED",
      title: tTenant("emails.messaging.requestCreatedTitle"),
      message: title,
...
export async function PATCH(req: Request) {
  ...
  if (!REQUEST_STATUS_SET.has(status)) return mobileError(t("adminDocs.api.requests.badStatus"))
  ...
  if (!existing) return mobileError(t("adminDocs.api.requests.notFound"), 404)
  ...
  if (existing.status !== status) {
    const { t: tTenant } = await getTForUser(existing.tenant.userId)
    await notifyUser({
      userId: existing.tenant.userId,
      type: "REQUEST_STATUS_CHANGED",
      title: tTenant("emails.messaging.requestStatusTitle"),
