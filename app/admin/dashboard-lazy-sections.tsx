...
export function DashboardLazySections({
  ...
  return (
    <>
      ...
      {/* Пустые карточки не показываем — «Нет активных заявок» на полэкрана не несёт пользы */}
      {(data.recentRequests.length > 0 || data.recentTasks.length > 0) && (
        <div className={`grid grid-cols-1 gap-4 ${data.recentRequests.length > 0 && data.recentTasks.length > 0 ? "md:grid-cols-2" : ""}`}>
      ...
      <ul className="divide-y divide-slate-50 dark:divide-slate-800/70">
        {items.map((item) => (
          <li key={item.id}>
            <Link
              href={href}
              className="flex items-center gap-3 px-5 py-2.5 text-sm transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/60"
            >
              <span className={`h-2 w-2 shrink-0 rounded-full ${STATUS_DOT[item.status] ?? "bg-slate-300"}`} />
              <span className="min-w-0 flex-1 truncate text-slate-700 dark:text-slate-300">{item.title}</span>
