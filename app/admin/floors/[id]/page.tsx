...
export default async function FloorEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Гейт: визуализация помещения — BETA-фича, доступна только на топ-тарифах
  const hasFloorEditor = await hasFeature(orgId, "floorEditor")
  if (!hasFloorEditor) {
    return (
      <div className="space-y-4">
        <Link href="/admin/spaces" className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-100 w-fit">
          <ArrowLeft className="h-4 w-4" />К помещениям
        </Link>
