...
const severityMeta: Record<Severity, { icon: typeof AlertTriangle; pill: string; iconBox: string }> = {
  critical: {
    icon: AlertTriangle,
    ...
    return (
      <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-500/30 dark:bg-emerald-500/10">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-emerald-950 dark:text-emerald-100">{t("adminSettings.dataQuality.relations.okTitle")}</h2>
            <p className="mt-1 text-sm text-emerald-700 dark:text-emerald-200">
              {t("adminSettings.dataQuality.relations.okText")}
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{t("adminSettings.dataQuality.relations.title")}</h2>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {t("adminSettings.dataQuality.relations.hint")}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-xs font-medium text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300">
            {t("adminSettings.dataQuality.relations.critical", { count: overview.summary.critical })}
          </span>
          <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
            {t("adminSettings.dataQuality.relations.warning", { count: overview.summary.warning })}
          </span>
          <span className="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-300">
            {t("adminSettings.dataQuality.relations.info", { count: overview.summary.info })}
          </span>
        </div>
      </div>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  {t(`adminChecks.integrity.contours.${contour.key}.label`)}
                </h3>
                <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">
                  {t(`adminChecks.integrity.contours.${contour.key}.description`)}
                </p>
              </div>
                ...
                return (
                  ...
                  >
                    <span className="min-w-0">
                      <span className="block truncate font-medium text-slate-900 dark:text-slate-100">{t(issue.titleKey)}</span>
                      <span className="mt-0.5 block line-clamp-2 text-xs text-slate-500 dark:text-slate-400">{t(issue.descriptionKey)}</span>
                    </span>
