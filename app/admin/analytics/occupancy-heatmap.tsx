  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="px-5 py-3.5 border-b border-slate-100">
        ...
        <p className="text-xs text-slate-500 mt-0.5">% времени помещение занято с начала года</p>
      </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  s.percent >= 90 ? "border-red-300" : s.percent >= 50 ? "border-amber-300" : "border-slate-200"
                )}
                style={{ background: bg }}
                title={`Каб. ${s.spaceNumber} · ${s.area} м² · занят ${s.percent}% времени`}
              >
                <p className="text-xs font-bold text-slate-800">{s.spaceNumber}</p>
                <p className={cn(
                  "text-[10px]",
                  s.percent >= 70 ? "text-white font-bold" : "text-slate-600"
                )}>
                  {s.percent}%
                </p>
              </div>
