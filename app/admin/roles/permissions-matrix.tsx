        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 sticky left-0 bg-slate-50 z-10">Раздел</th>
              {roles.map((r) => (
          ...
          <tbody>
            {sections.map((s) => (
              <tr key={s.key} className="border-b border-slate-100 hover:bg-slate-50/50">
                <td className="px-4 py-2 font-medium text-slate-700 sticky left-0 bg-white z-10">{s.label}</td>
                {roles.map((r) => {
