  ...
  return (
    <>
      <div className="flex items-center gap-2 shrink-0">
        ...
        {hasResponse && (
          <button onClick={() => setOpen(true)} className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:underline flex items-center gap-1">
            Изменить ответ
          </button>
        )}
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            >
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-1.5">Ответ администратора</label>
                <textarea
                  name="response"
                  required
                  rows={4}
