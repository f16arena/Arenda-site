...
export async function assignFullFloor(floorId: string, tenantId: string, fixedRent: number) {
  ...
  // Освобождаем индивидуальное помещение если было
  if (tenant.spaceId) {
  ...
  // Все помещения этажа помечаем как занятые
...
export async function unassignFullFloor(floorId: string) {
  ...
  // Освобождаем все помещения этажа (только те у которых нет индивидуального арендатора)
