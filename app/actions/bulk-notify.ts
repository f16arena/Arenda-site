...
export async function sendBulkNotificationToTenants(input: {
  ...
  if (!allowed) {
    return { ok: false, error: t("actions.bulkNotify.notAvailableOnPlan") }
  }

  const title = input.title?.trim() ?? ""
  const message = input.message?.trim() ?? ""
  if (!title || !message) return { ok: false, error: t("actions.bulkNotify.titleAndTextRequired") }
  ...
  for (const row of tenants) {
    if (!row.user?.id) { skipped++; continue }
    try {
      await notifyUser({
        userId: row.user.id,
        type: "BULK_INFO",
