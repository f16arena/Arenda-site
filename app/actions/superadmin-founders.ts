...
export async function updateFoundersState(input: {
  ...
  if (typeof input.totalSlots === "number") {
    if (input.totalSlots < 1 || input.totalSlots > 1000) {
      return { ok: false, error: t("actions.founders.badTotalSlots") }
    }
  ...
  if (typeof input.discountPct === "number") {
    if (input.discountPct < 0 || input.discountPct > 100) {
      return { ok: false, error: t("actions.founders.badDiscount") }
    }
    data.discountPct = Math.floor(input.discountPct)
  }
  if (Object.keys(data).length === 0) return { ok: false, error: t("actions.founders.noChanges") }
...
export async function releaseFoundersSlot(orgId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!org.isFoundersMember) return { ok: false, error: t("actions.founders.notMember") }
...
export async function grantFoundersSlot(orgId: string): Promise<{ ok: boolean; error?: string; slotNumber?: number }> {
  ...
  const r = await db.$transaction(async (tx) => {
    ...
    if (!org) return { ok: false as const, error: t("actions.common.organizationNotFound") }
    if (org.isFoundersMember) return { ok: false as const, error: t("actions.founders.alreadyMember") }
    if (org.plan?.code === "FREE") return { ok: false as const, error: t("actions.founders.freePlanExcluded") }

    const state = await tx.foundersProgramState.findUnique({ where: { id: "singleton" } })
    if (!state) return { ok: false as const, error: t("actions.founders.stateMissing") }
    if (state.takenSlots >= state.totalSlots) return { ok: false as const, error: t("actions.founders.noSlotsLeft") }
