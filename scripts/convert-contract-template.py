...
REPLACEMENTS = [
    ...
    # Более гибкий regex: разрешаем любой текст между «тариф*» и «тенге»,
    # с обязательными скобками и пробелами.
    (re.compile(r"тариф(?:у|ом)?\s+[_(\s)\d]+тенге\s+за\s+1\s*кв\.\s*м"),
