...
export function AdminSelectOrgView({ orgs, userName }: { orgs: SelectableOrg[]; userName: string }) {
  const { t } = useT()
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50">
        ...
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              {t("superadmin.selectOrg.platformMode")}
            </span>
          ...
          <Link href="/superadmin" className="text-xs font-medium text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300">
            {t("superadmin.selectOrg.backToPanel")}
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{t("superadmin.selectOrg.title")}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 max-w-md mx-auto">
            {t("superadmin.selectOrg.subtitle")}
          </p>
        </div>
        ...
        {orgs.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-10 text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">{t("superadmin.selectOrg.empty")}</p>
            <Link href="/superadmin/orgs/new" className="inline-block mt-4 rounded-lg bg-purple-600 hover:bg-purple-700 px-4 py-2 text-sm font-medium text-white">
              {t("superadmin.selectOrg.createFirst")}
            </Link>
          </div>
        ) : (
...
function OrgCard({ org }: { org: SelectableOrg }) {
  ...
  const status: { label: string; className: string } = !org.isActive
    ? { label: t("superadmin.selectOrg.statusInactive"), className: "bg-slate-200 text-slate-600 dark:text-slate-400" }
    : org.isSuspended
    ? { label: t("superadmin.selectOrg.statusSuspended"), className: "bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300" }
    : isExpired
    ? { label: t("superadmin.selectOrg.statusExpired"), className: "bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300" }
    : daysLeft !== null && daysLeft <= 7
    ? {
        label: t("superadmin.selectOrg.statusExpiring", { count: daysLeft }),
        className: "bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300",
      }
    : { label: t("superadmin.selectOrg.statusActive"), className: "bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300" }
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 text-center">
          <div className="rounded-lg bg-slate-50 dark:bg-slate-800/50 py-2">
            <p className="text-[10px] text-slate-500 dark:text-slate-400">{t("superadmin.selectOrg.statPlan")}</p>
            <p className="text-xs font-semibold text-slate-900 dark:text-slate-100 mt-0.5">{org.planName ?? "—"}</p>
          </div>
          <div className="rounded-lg bg-slate-50 dark:bg-slate-800/50 py-2">
            <p className="text-[10px] text-slate-500 dark:text-slate-400">{t("superadmin.selectOrg.statBuildings")}</p>
            <p className="text-xs font-semibold text-slate-900 dark:text-slate-100 mt-0.5">{org.buildingsCount}</p>
          </div>
          <div className="rounded-lg bg-slate-50 dark:bg-slate-800/50 py-2">
            <p className="text-[10px] text-slate-500 dark:text-slate-400">{t("superadmin.selectOrg.statUsers")}</p>
        ...
        <div className="flex gap-2">
          <ConfirmDialog
            title={t("superadmin.selectOrg.loginTitle", { name: org.name })}
            description={t("superadmin.selectOrg.loginDescription")}
            confirmLabel={t("superadmin.orgs.loginAs")}
            onConfirm={() => {
              if (!org.hasOwner) {
                toast.error(t("superadmin.selectOrg.noOwner"))
                return
              }
              startTransition(async () => {
                try {
                  await impersonateOrg(org.id)
                  toast.success(t("superadmin.selectOrg.loggingIn"))
                  router.push("/admin")
                  router.refresh()
                } catch (e) {
            ...
            trigger={
              ...
              >
                ...
                {t("superadmin.selectOrg.loginAsClient")}
              </button>
            }
          />
          <button
            ...
            disabled={pending || !org.isActive}
            title={t("superadmin.selectOrg.viewTitle")}
          ...
          <Link
            ...
            title={t("superadmin.selectOrg.openCard")}
          >
            ⋯
          </Link>
        </div>
