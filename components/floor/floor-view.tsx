...
export function FloorView({
  ...
  return (
    <div className="relative bg-slate-50 rounded-xl border border-slate-200 overflow-hidden h-[500px]">
      <svg
        ref={svgRef}
      ...
      {/* Zoom controls */}
      <div className="absolute top-3 right-3 bg-white rounded-lg border border-slate-200 shadow-sm p-1 flex flex-col gap-0.5">
        <button onClick={() => setZoom((z) => Math.min(3, z * 1.2))} className="p-1.5 hover:bg-slate-100 rounded">
          <ZoomIn className="h-4 w-4 text-slate-600" />
        </button>
        <button onClick={() => setZoom((z) => Math.max(0.3, z / 1.2))} className="p-1.5 hover:bg-slate-100 rounded">
          <ZoomOut className="h-4 w-4 text-slate-600" />
        </button>
        <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }) }} className="p-1.5 hover:bg-slate-100 rounded">
      ...
      {selectedSpace && selectedEl && (
        <div className="absolute right-3 bottom-3 w-72 bg-white rounded-xl border border-slate-200 shadow-xl">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-900">Каб. {selectedSpace.number}</p>
          ...
          <div className="p-4 text-sm space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-900">{selectedSpace.area} м²</span>
            </div>
            <div className="flex justify-between">
            ...
            {selectedSpace.tenant ? (
              <>
                <div className="border-t border-slate-100 pt-2">
                  <p className="text-xs text-slate-400 mb-0.5">Арендатор</p>
                ...
                {selectedSpace.tenant.contractEnd && (
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">Договор до:</span>
              ...
              <p className="text-xs text-slate-400 italic border-t border-slate-100 pt-2">Помещение свободно</p>
            )}
            {selectedSpace.description && (
              <p className="text-xs text-slate-500 italic border-t border-slate-100 pt-2">{selectedSpace.description}</p>
            )}
          </div>
        </div>
      )}
