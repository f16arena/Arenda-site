...
export function detectStatus(space: SpaceInfo | undefined): string {
    ...
    <div ref={containerRef} className={`relative bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden ${view === "3d" ? "h-[440px]" : "h-[260px]"}`}>
      ...
      <div className="absolute top-3 left-3 z-10 flex rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <button
          type="button"
          onClick={() => setView("2d")}
          className={`flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium ${view === "2d" ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900" : "text-slate-600 dark:text-slate-400"}`}
        >
        ...
        <button
          type="button"
          onClick={() => setView("3d")}
          title="Объёмный вид: генерируется из плана автоматически, вращение мышью"
          className={`flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium ${view === "3d" ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900" : "text-slate-600 dark:text-slate-400"}`}
        >
      ...
      {/* Hover tooltip: номер, площадь, кем занято (только 2D) */}
      {view === "2d" && hoveredSpace && hovered && hovered.elId !== selected && (
        <div
