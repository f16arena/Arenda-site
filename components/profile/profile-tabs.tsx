          ...
          {phone !== undefined && (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                ...
                <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t("common.profile.phone")}</h2>
              </div>
              <div className="p-5 text-sm">
                {phone ? (
                  <p className="font-mono text-slate-900 dark:text-slate-100">{phone}</p>
                ) : (
                  <p className="text-slate-500 dark:text-slate-400">{t("common.state.notSet")}</p>
                )}
                <p className="text-xs text-slate-400 dark:text-slate-400 mt-2">
                  {t("common.profile.phoneHint")}
                </p>
              </div>
            </div>
          )}
