...
const CATEGORY_META: Record<Category, { icon: React.ElementType; label: string; color: string }> = {
  ...
  other: { icon: Settings, label: "Прочее", color: "text-slate-500" },
}
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-8 z-40 w-[420px] bg-white rounded-2xl shadow-2xl border border-slate-200 max-h-[600px] flex flex-col overflow-hidden">
            {/* Header */}
  ...
  const Inner = (
    <div className={cn("px-5 py-3 hover:bg-slate-50 border-b border-slate-100 group", !item.isRead && "bg-blue-50/30")}>
      <div className="flex items-start gap-3">
        <div className={cn("mt-0.5 inline-flex h-7 w-7 items-center justify-center rounded-lg shrink-0", "bg-slate-100", meta.color)}>
