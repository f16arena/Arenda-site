...
export function ChatView({ currentUserId, contacts, messagesByContact, showBroadcast }: ChatViewProps) {
  ...
  return (
    <div className="grid grid-cols-[280px_1fr] gap-0 h-[calc(100vh-180px)] bg-white rounded-xl border border-slate-200 overflow-hidden">
      {/* Sidebar */}
      <div className="border-r border-slate-200 flex flex-col overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
          <p className="text-sm font-semibold text-slate-700">Контакты</p>
        </div>
        <div className="flex-1 overflow-y-auto">
          {showBroadcast && (
            <button
              ...
              className={cn(
                "w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-slate-50 border-b border-slate-100",
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900">Всем (объявление)</p>
                <p className="text-xs text-slate-500">Отправить всем участникам</p>
              </div>
            </button>
          )}

          {contacts.length === 0 && !showBroadcast && (
            <p className="px-4 py-8 text-center text-sm text-slate-400">Нет контактов</p>
          )}
          ...
          {contacts.map((c) => (
            ...
            >
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-200 shrink-0">
                <span className="text-xs font-bold text-slate-600">{c.name[0]?.toUpperCase()}</span>
              </div>
              <div className="flex-1 min-w-0">
      ...
      <div className="flex flex-col overflow-hidden">
        {!selectedId ? (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-sm text-slate-400">Выберите контакт чтобы начать</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
              {selectedId === BROADCAST_ID ? (
                <>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900">Объявление всем</p>
                    <p className="text-xs text-slate-500">Сообщение придёт каждому участнику</p>
                  </div>
                </>
              ) : selectedContact ? (
                <>
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-200">
                    <span className="text-xs font-bold text-slate-600">{selectedContact.name[0]?.toUpperCase()}</span>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{selectedContact.name}</p>
                    <p className="text-xs text-slate-500">{ROLE_LABELS[selectedContact.role] ?? selectedContact.role}</p>
                  </div>
                </>
              ) : null}
            </div>
              ...
              {selectedId === BROADCAST_ID ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center max-w-sm">
                    ...
                    <p className="text-sm font-medium text-slate-700">Режим объявления</p>
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-sm text-slate-400">Нет сообщений. Напишите первым.</p>
                </div>
              ) : (
                messages.map((m) => {
                    ...
                    <div key={m.id} className={cn("flex", isMine ? "justify-end" : "justify-start")}>
                      <div
                        className={cn(
                          ...
                          isMine ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-900"
                        )}
                      >
                        {m.subject && (
                          <p className={cn("text-xs font-semibold mb-1", isMine ? "text-blue-100" : "text-slate-500")}>
                            {m.subject}
                          </p>
                        )}
                        <p className="text-sm whitespace-pre-wrap break-words">{m.body}</p>
                        <div className={cn("text-[10px] mt-1 flex items-center gap-2", isMine ? "text-blue-100" : "text-slate-400")}>
                        ...
                        {isMine && (
                          <button
                            ...
                            className="absolute -top-2 -right-2 h-6 w-6 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition flex items-center justify-center"
                            aria-label="Удалить"
                          >
            ...
            >
              {selectedId === BROADCAST_ID && (
                <input
                  ...
                  className="w-full mb-2 rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              )}
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none resize-none"
                  onKeyDown={(e) => {
              ...
              <p className="text-[10px] text-slate-400 mt-1">Enter — отправить, Shift+Enter — новая строка</p>
            </form>
          </>
        )}
      </div>
