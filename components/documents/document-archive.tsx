...
export async function DocumentArchive({ organizationId, documentType, period }: Props) {
  ...
  if (docs.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-8 text-center print:hidden">
        <FileText className="h-8 w-8 text-slate-300 mx-auto mb-2" />
        <p className="text-sm text-slate-400">
          Архив пуст{period ? " за выбранный период" : ""}.
        </p>
        <p className="text-[11px] text-slate-400 mt-1">Сгенерированные документы будут появляться здесь.</p>
      </div>
    )
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden print:hidden">
      ...
      <table className="w-full text-sm">
        <thead className="bg-slate-50">
          <tr>
            <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-widest">№</th>
            <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-widest">Контрагент</th>
            <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-widest">Период</th>
            <th className="px-4 py-2 text-right text-[10px] font-medium text-slate-500 uppercase tracking-widest">Сумма</th>
            <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-widest">Дата</th>
        ...
        <tbody>
          {docs.map((d) => (
            <tr key={d.id} className="border-t border-slate-50 hover:bg-slate-50/50">
              ...
              <td className="px-4 py-2 text-slate-700">{d.tenantName}</td>
              <td className="px-4 py-2 text-slate-600">{d.period ?? "—"}</td>
