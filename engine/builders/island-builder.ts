...
export function buildIsland(island: Island, parent: TransformNode, scene: Scene, reg: MaterialRegistry, lite = false): TransformNode {
  ...
  const faceH = island.kind === "counter" ? h * 0.45 : h * 0.62
  const faceY = island.kind === "counter" ? h * 0.72 : h * 0.55
  const face = MeshBuilder.CreateBox(`isl_face_${island.id}`, { width: w * 0.86, height: faceH, depth: 0.03 }, scene)
