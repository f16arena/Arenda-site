...
## Этап 2. Машинная зачистка копипаста — простые зоны ✅ (22–23.07, ~305 замен)

Готово (агент-проходы, только вёрстка, логика не тронута): cabinet (43),
admin/finances (~35), admin/documents+contracts (22), admin/tenants (~40),
обслуживание requests/tasks/complaints/emergency/calendar/faq (40),
buildings/spaces/meters/service-fee (34), superadmin (~60), auth+публичные (~30).
Исключено из зон и ЖДЁТ структурной волны: таблицы <table>, модалки fixed inset-0,
нативные select, чат messages, редактор этажа floors, 3D builder, конструкторы
документов, семантические цветные кнопки (emerald/amber/teal), ссылки-кнопки.

## Этап 2b. Структурная волна 🟡 (медленно, с ручной проверкой экранов)

- [~] Модалки fixed inset-0 → Dialog/AlertDialog. Порция 1 (пилот, 5 шт, 2a8a2bc),
      порция 2 (ConfirmDialog + зона «арендаторы», 8 файлов, 9672a84).
      Осталось 26 файлов, из них 6 — боковые панели и шторки (пойдут на Sheet).
- [ ] Таблицы (62 шт) → Table (+обёртки overflow)
- [ ] Нативные select → Select — нужен пилот на одном экране с проверкой глазами;
      примитив Select в проекте пока не применялся ни разу
- [ ] Семантические кнопки: добавить варианты (success/warning) в Button и перевести
- [ ] Ссылки-кнопки → Button asChild

**Порция 2 (11.09.2026).** Главный рычаг — общий `components/ui/confirm-dialog.tsx`
переписан на Dialog изнутри при неизменном публичном API: это починило все 26 мест
вызова без правок в них. Плюс 7 модалок зоны «арендаторы». В каждой поставлен guard:
пока идёт отправка, Esc и клик мимо не закрывают окно.

## Этап 2 (исходный охват — для справки) 
...
## Этап 3. Редизайн экранов 🟡 (эталон готов, прогон не начат)
...
**Эталон (11.09.2026).** Девять ключевых экранов собраны как дизайн-макеты на реальных
токенах из `app/globals.css` (Manrope, oklch, радиусы 6/8/10/14, кнопки 36/32 px,
карточки `rounded-xl` + `ring-1`, бейджи-пилюли 20 px): обзор, список арендаторов,
карточка арендатора, финансы, документы и договоры, план этажа, кабинет арендатора,
должности и права, обзор в тёмной теме. Ссылка и исходники — в vault,
заметка `Commrent/План выходных 12-13 сентября`.

Открытые вопросы по эталону: плотность строк таблиц (сейчас 44 px), паттерн для
нативных select, варианты success/warning в Button для семантических кнопок.
...
- 22.07.2026 — аудит завершён (страницы/дубли, UI-консистентность, навигация), план утверждён,
  старт этапа 0.
- 11.09.2026 — этап 2b порция 2 (ConfirmDialog + «арендаторы»); собран эталон этапа 3 из 9 экранов
- 19.09.2026 — этап 3 начат с обзора `/admin` (коммит `a790507`): шапка + 4 цифры, одна строка
  незаконченной настройки, «Этот месяц» (начисления → счета → АВР → оплаты с пояснениями и
  кнопками), «Требует внимания»; убраны дубли (график денег, список арендаторов, заявки/задачи,
  «Пульс», ссылка на календарь). Меню: «Заявки и жалобы» (хаб SERVICE_TABS), «Счётчики» → Финансы,
  «Здоровье платформы» → Настройки, эксплуатационный сбор — только в карточке здания
  (`/admin/service-fee` → redirect на `/admin/buildings`).
- 19.09.2026 — календарь (`92a7bdf`): все 4 пути привязки арендатора, дни по Asia/Almaty, без
  дубля «оплаченное начисление + оплата», итог месяца, текст в клетках, «Впереди» с сегодня.
- 19.09.2026 — здания (`90741e2`): арендаторы по 4 путям, помещения без объектов (объекты
  отдельной цифрой), площадь без крыши/территории, сбор ставкой, подписи контактов,
  «План с арендаторами» / «3D-модель».
- 19.09.2026 — помещения (`7a8a870` → по отзыву владельца `0b18273`): плитки по этажам, свободное
  зелёным с «Заселить», 3 цифры, переключатель «Все / Только свободные», крыша и территория
  своими разделами, «Очистить всё» в «Опасной зоне». Таблица с фильтрами владельцу не зашла
  («перегружено, скучно») — не возвращаться к ней.
- 19.09.2026 — арендаторы (`d75cf88`, `892e73f`): цифры «долг / договор кончается / без подписанного»,
  колонки «Аренда в месяц» и «Договор до», фильтр по договорам, «Заселить арендатора» /
  «Только карточка», короткие названия (lib/company-name.ts), фильтр здания по 4 путям.
