...
- ВАЖНО: прокси запущен под `User=ubuntu` — ТАКИМ ЖЕ, как jar (esf-sign тоже User=ubuntu). Если запустить под root — jar (ubuntu) не прочитает временный .p12 → `FileNotFoundException ... (Permission denied)`. (Поймали и исправили 15.06.2026.)
