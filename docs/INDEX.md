...
- [[AUDIT_2026-05-29]] — продолжение: 7 критов 26 мая закрыты, исправлена касса+restore, открыты push/kaspi/bank-import

### Казахская версия
- [[i18n-glossary]] — термины и аббревиатуры: перевод по смыслу, ЖСН/БСН/ҚҚС/ЭШФ
- [[i18n-documents-plan]] — договор, АВР, счёт, акт сверки: без вычитки юриста не катим
- [[i18n-content-plan]] — справка (нужна миграция БД), блог, юридические страницы

### Визуализация зданий
- [[indoor-map/SPEC]] — **источник правды**: indoor-карта по образцу 2ГИС (план этажа + объём из того же плана)
- [[building-studio/SPEC]] — архив: замороженное ТЗ игрового Build Mode на Babylon.js
- [[building-studio/ROADMAP]] — архив: фазы замороженного ТЗ

## 📓 Журнал работы

Ведётся **вне репозитория** — в личном Obsidian-vault, рядом с заметками других проектов:
`C:\Users\Арыстан\Documents\Obsidian Vault\Commrent\Журнал\YYYY-MM-DD.md`.

Пишется автоматически: хук `Stop` в `.claude/settings.json` после каждого ответа Claude Code
вызывает `../scripts/vault-journal.mjs`, который дописывает время, ветку, коммит, список
изменённых файлов и итог ответа.

- Короткие реплики (без правок файлов и короче 120 символов) не записываются — иначе журнал зарастает шумом
- Повторный `Stop` в той же сессии дубль не создаёт (состояние в `.claude/journal-state.json`)
- Пароли, токены, JWT и строки подключения затираются перед записью
- Путь к vault переопределяется переменной `OBSIDIAN_VAULT`
- Выключить: убрать блок `hooks.Stop` из `.claude/settings.json` (или `/hooks` в Claude Code)
...
## 🗺️ Графы кода (Mermaid)

Зависимости TypeScript-модулей, автогенерация через [madge](https://github.com/pahen/madge) → Mermaid. Открываются в Obsidian как интерактивные диаграммы.

- [[code-graphs/01-tenant-card]] — карточка арендатора `/admin/tenants/[id]` (23 файла)
- [[code-graphs/02-server-actions]] — server actions слой `app/actions/` (58 файлов)
- [[code-graphs/03-lib-core]] — библиотеки core `lib/` (102 файла, большой граф)

**Обновить графы:** `node scripts/gen-code-graph.mjs <папка> <output.md> "<title>"`
