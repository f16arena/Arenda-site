...
/**
 * Подписать XML тикета авторизации по стандарту xmlDsig (enveloped XML-подпись)
 * через сервис подписи КГД (esf_local_server.jar, операция documentXmlSignatureRequest).
 *
 * Отличие от signAwpXml: здесь нужен НЕ отдельный raw-base64 signature, а ПОЛНЫЙ
 * подписанный XML тикета — со встроенными <X509Certificate> и <SignatureValue>
 * (по стандарту xmlDsig). Этот XML целиком уходит в createSessionSigned как
 * signedAuthTicket (новый флоу ГОСТ-2015: тикет → подпись → сессия).
 *
 * TODO(SDK): сверить с эталонным esf_local_server.jar точные имена —
 *   операция documentXmlSignatureRequest, поля запроса (signableData/xml) и
 *   элемент ответа с подписанным XML (signedXml/signature/result).
 */
export async function signTicketXmlDsig(ticketXml: string): Promise<string> {
  if (!esfSignerConfigured()) {
    throw new Error(
      "Подпись для ИС ЭСФ ещё не подключена: разверните esf_local_server.jar на VPS и задайте "
      + "ESF_SIGN_URL / ESF_SIGN_CERT_PATH / ESF_SIGN_CERT_PIN.",
  ...
  const envelope = `<?xml version="1.0" encoding="UTF-8"?>`
    ...
    + `<signableData>${escXml(ticketXml)}</signableData>`
    + `<certificatePath>${escXml(ESF_SIGN_CERT_PATH)}</certificatePath>`
    + `<certificatePin>${escXml(ESF_SIGN_CERT_PIN)}</certificatePin>`
    + `</esf:documentXmlSignatureRequest>`
    + `</soapenv:Body></soapenv:Envelope>`
  ...
  // Подписанный XML тикета (xmlDsig): имя поля уточняется по SDK.
  const signed = pick(xml, "signedXml") || pick(xml, "signedData")
    || pick(xml, "signature") || pick(xml, "result") || pick(xml, "return")
  if (!signed) {
    const fault = pick(xml, "faultstring")
    throw new Error(fault ? `Сервис подписи (xmlDsig): ${fault}` : "Сервис подписи вернул ответ без подписанного XML тикета")
  }
  return signed
}
