// Справочник placeholder'ов загружаемых шаблонов документов. Интерфейс этой
// возможности удалён (её заменил конструктор), рендер в lib/template-engine
// остался. Подписи ниже НЕ отрисовываются, поэтому остаются русскими: если
// экран шаблонов вернут, они переедут в словарь вместе с ним.
...
export const PLACEHOLDER_DOCS: Record<DocumentType, { key: string; label: string }[]> = {
  CONTRACT: [
    ...
    // Готовые «представительские» фразы с правильной грамматикой (склонение
    // фамилий в род. падеж, без «в лице ИП Х» для физлиц-предпринимателей).
    { key: "landlord_intro", label: "Шапка арендодателя: полное имя + «в лице директора Иванова И.И., действующего на основании Устава»" },
    { key: "tenant_intro", label: "Шапка арендатора: полное имя + «в лице директора...» (для ТОО/АО) либо «действующий на основании Уведомления» (для ИП/ЧСИ)" },
